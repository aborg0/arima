...
case class Order(p: Int, q: Int, d: Int)

case class Seasonal(order: Order, period: Option[Int])
...
case class ArimaResult[V: Eq : Field](coefficients: IndexedSeq[V], sigma2: V, varCoef: V, mask: BitSet,
                                      logLikelihood: V, aic: V, arma: IndexedSeq[Int], residuals: IndexedSeq[V],
                                      series: IndexedSeq[V], convergenceCode: Int, numConditionals: Int,
                                      numObservations: Int, model: ArimaInternal[V])

/**
  * Created by aborg on 20/05/2017.
  */
case class Arima[V: Eq : Field](x: IndexedSeq[V], order: Order = Order(0, 0, 0),
                                seasonal: Seasonal = Seasonal(Order(0, 0, 0), None),
                                xreg: Option[Mat[V]] = None, includeMean: Boolean = true,
                                transformPars: Boolean = true, fixed: Option[IndexedSeq[V]] = None,
                                init: Option[IndexedSeq[Int]] = None,
                                method: ArimaLearningMethod = CssMl, nCond: Option[Int],
                                ssInit: SsInit = Rossignol2011,
                                optimMethod: OptimizationMethod = BFGS,
                                optimControl: Seq[Any] = Seq(), kappa: Double = 1e6) {
